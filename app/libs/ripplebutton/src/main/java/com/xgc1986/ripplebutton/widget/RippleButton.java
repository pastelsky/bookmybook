...
public class RippleButton extends Button {
    ...
    private int rippleColorIs = 0x40000000;
    ...
    public void setColors(int buttonColor, int controlHighlightColor) {
        ...
        rippleColorIs = controlHighlightColor;
        setButtonColor(buttonColor);
        setrippleColorIs(controlHighlightColor);
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public void setrippleColorIs(final int color) {
        rippleColorIs = color;
    ...
    private void manageAttibuteSet(AttributeSet attrs) {
        if (!isInEditMode()) {
            ...
            int hColor = a.getColor(R.styleable.RippleButton_rippleColorIs, rippleColorIs);

            a.recycle();

            buttonColor = nColor;
            rippleColorIs = hColor;
            setButtonColor(nColor);
            setrippleColorIs(hColor);
        }
    }
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
            if (event.getActionMasked() == MotionEvent.ACTION_UP) {
                ...
                if (rippleColorIs != 0) {
                    ...
                    getBackground().setColorFilter(rippleColorIs, PorterDuff.Mode.MULTIPLY);
                }
            }
        }
